...
const PERIODS = ["1990s", "2000s", "2010s", "2020s"] as const;

export default function ArchiveCatalogueDemo() {
...
function SensitivityBadge({ level }: { level: string }) {
  const colour = level === "Open" ? "bg-emerald-50 border-emerald-200 text-emerald-800" : level === "Restricted" ? "bg-amber-50 border-amber-200 text-amber-800" : "bg-rose-50 border-rose-200 text-rose-800";
  return (
    <span className={`text-xs px-2 py-1 rounded-full border ${colour}`}>{level}</span>
  );
}
...
function Select({ label, value, onChange, options }: { label: string; value: string; onChange: (v: string) => void; options: (string | readonly string[])[]; }) {
  return (
    <div>
      ...
      >
        {options.map((opt, idx) => (
          <option key={idx} value={typeof opt === "string" ? opt : (opt as any)}>
            {opt === "" ? "Any" : (opt as any)}
          </option>
        ))}
      </select>
...
function DetailModal({ item, onClose, onRequest, children }: { item: any; onClose: () => void; onRequest: () => void; children?: React.ReactNode; }) {
  return (
...
function RequestForm({ itemId, onCancel, onSubmit }: { itemId: string; onCancel: () => void; onSubmit: () => void; }) {
  return (
    <form
...
function LabeledInput({ id, label, type = "text", required = false, placeholder = "" }: { id: string; label: string; type?: string; required?: boolean; placeholder?: string; }) {
  return (
    <div>
